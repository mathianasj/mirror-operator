...
Environment Variables:
    ...
    CLI_INSTALL_DIR               Directory for CLI tools (default: ~/.local/bin)
...
install_cli_tools() {
    ...
    if [ -f "${CLI_TOOLS_DIR}/openshift-client-linux.tar.gz" ]; then
        ...
        if [ ${oc_choice} -eq 0 ]; then
            ...
            if [ -f "$temp_dir/oc" ]; then
                ...
                log "Installed oc: $(${CLI_INSTALL_DIR}/oc version --client 2>/dev/null || echo 'version check failed')"
            fi
    ...
    # Extract and install openshift-install
...
create_cli_symlinks() {
    log "Creating symbolic links to CLI tools in script directory..."

    local created_links=false

    # Create symlinks for each installed tool
    for tool in oc kubectl openshift-install oc-mirror; do
        if command -v "${tool}" &>/dev/null; then
            local tool_path=$(which "${tool}")
            local link_path="${SCRIPT_DIR}/${tool}"
            ...
            # Create symlink
            if ln -s "${tool_path}" "${link_path}" 2>/dev/null; then
                log "  Created symlink: ${tool} -> ${tool_path}"
                created_links=true
            else
                log "  WARNING: Could not create symlink for ${tool}"
            fi
        fi
    done

    if [ "${created_links}" = false ]; then
        log "  No CLI tools found to link"
    fi
}
...
check_mirror_registry() {
    ...
    if command -v systemctl &>/dev/null && systemctl is-active --quiet quay-pod.service 2>/dev/null; then
        log "Mirror-registry is already running (systemd)"
        return 0
    fi

    # Check if podman containers exist
    if podman ps -a --format '{{.Names}}' 2>/dev/null | grep -q '^quay$'; then
        log "Mirror-registry container exists"
        return 0
    fi
...
start_containers() {
    ...
    # Ensure all directories have proper permissions for container to write (recursive)
    find "${SCRIPT_DIR}" -type d -exec chmod 777 {} \;
    ...
    if command -v chcon &>/dev/null; then
        ...
        chcon -Rt container_file_t "${SCRIPT_DIR}" || log "Warning: chcon failed (SELinux may not be enabled)"
    fi
...
mirror_to_registry() {
    ...
    if oc-mirror --v2 --from "file://${archives_dir}" --config "${imageset_config}" ${oc_mirror_args} "docker://${registry_url}"; then
        ...
        if [ -f "${SCRIPT_DIR}/idms-oc-mirror.yaml" ] || [ -f "${SCRIPT_DIR}/itms-oc-mirror.yaml" ]; then
            log ""
            log "Mirror complete! To configure your cluster to use the mirrored content:"
            log ""
            if [ -f "${SCRIPT_DIR}/idms-oc-mirror.yaml" ]; then
                log "  kubectl apply -f ${SCRIPT_DIR}/idms-oc-mirror.yaml"
            fi
            if [ -f "${SCRIPT_DIR}/itms-oc-mirror.yaml" ]; then
                log "  kubectl apply -f ${SCRIPT_DIR}/itms-oc-mirror.yaml"
            fi
            log ""
        fi
    else
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            exit 0
            ;;
        start|stop|restart|status|logs|mirror|clean)
            COMMAND="$1"
            shift
            break
            ;;
